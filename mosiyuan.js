'use strict';
{
	...
	const ZPC_233='<span class="siyuan">ZPC_233</span>';
	const DAILY_MESSAGES=[
		`你强归你强，${ZPC_233}比你强！`,
		`你可以按住<code>Enter</code>来快速点击一个按钮。`,
		`${ZPC_233}是我们的红太阳，没有她我们会死！`,
		`你<strong>不能</strong>通过按住<code>Shift</code>点击一个按钮以直接购买最大数量的事物。`,
		`今天又是美好的一天~`,
		`若不是${ZPC_233}相助，也许我到现在还不知道我做错了什么。现在我知道了，是<code>LCT</code>上<code>splay</code>的时候判断父亲是否为根直接用了<code>if(fa->fa)</code>。`,
		`洁白的光点游走于漆黑的天幕之上，暗紫色的薄纱在其间舞动。在此无月之夜，是否可以知晓：“后缀自动机怎么写？”`,
		`那些文档有一部分真的有用……`,
		`${ZPC_233} 膜 她 自 己`,
		`${ZPC_233} D 她 自 己`,
		`如果你花掉了一些东西，它的花费也会降低，就像你从没买过它一样。`,
		`要想探寻真理，不仅要虔诚膜拜${ZPC_233}，领导者的实力也是很重要的。`,
		`<code>code.replace(/\\n\\s*\\{/mg,\'{\');</code>`,
		`嘤嘤嘤`,
		`Hello world!`,
		`在夕阳的照耀下，教堂的圆顶散发出金色的光辉。`,
		`这一刻，我们都是${ZPC_233}的忠实粉丝。`,
		`Do the magic!`,
		`传说${ZPC_233}会在被她抛弃的终端上用 root 权限执行<code>rm -rf /*</code>。`,
		`${ZPC_233}太强了，所以你复制不了这个网站中的任何一个${ZPC_233}。`,
		`空谈误国，实干兴邦。`,
// 		`<img src="./daily1.png">`,
// 		`<img src="./daily2.png">`,
// 		`<img src="./daily3.png">`,
// 		`<img src="./daily4.png">`,
		...
		`窝 又 被 ${ZPC_233} D 了 QAQ`,
		`道路千万条，光明第一条。防御没做好，黑屏两行泪。`,
		`元素和光明在你下线时也会增加！`,
		`贪 D 的(9^0+9^1+...+9^n+...)头 ${ZPC_233}`,
		`${ZPC_233}:“辣鸡，真辣鸡！”`,
		`${ZPC_233}:“泥萌怎么这么菜 nya？”`,
		`${ZPC_233}:“我就 D 你怎么了？”`,
		`${ZPC_233}:“哪场 ** CF 连样例解释都没有？”`,
		`${ZPC_233}:“tourist 能过，那窝肯定也能过！”`,
		`萌新三连:“窝怎么立直了 nya？胡是什么 nya，可以跳过吗？自摸是不是每巡都有的，好烦 nya！”`,
		`如果出现了难以对付的敌人，你可以<strong>召唤拯救</strong>扫清他们。`,
		`2019-08-21 ${ZPC_233} 被阿祭`,
		`${ZPC_233} AK NOI！`,
		`${ZPC_233} AK IOI！`,
		`${ZPC_233} 女装！`,
		`<strong>传教</strong>可以增加你的信徒数量。`,
		`传教中出现的题目来自某场${ZPC_233}将要 AK 的比赛的笔试。`,
		`强迫症真的这么好吗？`,
		`JRNEOJ 不是 OJ！`,
	...
	const SP_PROBLEMS=[
		...
		['在 Linux 系统中，文件夹中的文件可以与该文件夹同名吗：可以','不行',`只有${ZPC_233}才能做到`,'我不知道'],
		...
		[`使用 Reset 键强行重新启动计算机可能会对系统造成的后果是：文件系统损坏、膜拜${ZPC_233}存档损坏`,'中央处理器损坏','硬盘损坏'],
...
-rwx------ 1 user None 144  Jul 2 16:35 foobar.sh
	...
	其中，可以执行的文件是：foobar.sh`,
		...
		['选手在 NOI 机试过程中是否禁止使用网络：是','否',`只有${ZPC_233}可以`,'使用前需向监考人员举手示意'],
		['为程序 my.c 创建一个备份 myc.bak 时，使用的命令是：cp my.c myc.bak','he my.c my.bak','mv -c my.bak my.c','copy my.c my.bak'],
		['调试程序的方法有：单步调试、使用 print 类语句打印中间结果、读源代码','向监考人员求助',`膜拜${ZPC_233}以寻求帮助`],
		['在考试过程中，如果出现系统死机或者崩溃现象，选手应当采取的措施是：举手示意监考人员处理','关机重启',`向${ZPC_233}祈祷`,'使用旁边的电脑'],
		['提交的答案程序中如果包含 NOI 考试明确禁止使用的代码，后果是：本题成绩以 0 分计算','本场比赛成绩以 0 分计算','受到禁赛处罚','以评测机分数为准'],
		['对评测结果有疑义，需要申请复评，则：提出书面申请；经领队、科学委员会主席签字确认后，提交至评测人员','在调整程序之后',`在膜拜${ZPC_233}之后`],
		...
		['在 NOI 正式考试中如何登录自己的比赛用机：使用考前工作人员下发的账户及密码',`使用膜拜${ZPC_233}的账户及密码`,'使用试机时使用的账户及密码','使用考试开始时公布的账户及密码'],
		...
		['对试题理解有问题，选手可以：举手求助','将错就错',`膜拜${ZPC_233}`,'谨慎猜测'],
		...
		['草稿纸用完了，如何处理：举手向监考人员求助','使用自带的草稿纸','直接将草稿打在桌面上',`膜拜${ZPC_233}`],
		['水喝完了，如何处理：举手向监考人员再要一瓶','到饮水机处自行灌水','渴着','从其他选手处得到水瓶'],
		[`${ZPC_233}觉得考试太简单，能提前离开吗？能`,'不能','将题目全部完成方可离场',`只有${ZPC_233}能提前离场`],
		[`${ZPC_233}离开考场后，发现有个题可以吊打标算，能回去再改吗？不能`,'能','向监考人员说明情况后可返回考场','向科学委员会提交书面申请并通过后可返回考场'],
		...
		['IT 表示：信息技术','工业技术','电脑技术',`膜拜${ZPC_233}技术`],
		...
		['现代计算机所应用的存储程序原理是谁提出的？冯·诺依曼','艾伦·麦席森·图灵',`${ZPC_233}`,'Gennady Korotkevich'],
